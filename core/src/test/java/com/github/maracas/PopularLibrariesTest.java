...
/**
 * Run Maracas delta & detections on some popular libraries from Maven Central
 * and run some basic checks on the resulting models.
 * The old version of the library is used as client for detection purposes.
 */
class PopularLibrariesTest {
  ...
  void test_Maven_Library_Delta_and_Detections(String gid, String aid, String v1, String v2) {
    ...
    System.out.println("Found %s breaking changes and %d detections in %s:%s (%s -> %s)".formatted(
      result.delta().getBrokenDeclarations().size(), result.allDetections().size(), gid, aid, v1, v2));
  ...
  void assertIsValid(AnalysisResult res) {
    ...
    Collection<Detection> ds = res.allDetections();

    assertThat(delta, is(notNullValue()));
    delta.getBrokenDeclarations().forEach(bc -> {
